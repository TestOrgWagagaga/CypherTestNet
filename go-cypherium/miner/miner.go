...
import (
	...
	recfg "github.com/cyphertestnet/go-cypherium/reconfig/consensor"
)
...
type Miner struct {
	...
	// state        *state.StateDB
	// logs         []*types.Log
	consensus *recfg.Consensus
}

func New(eth Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, txblockchain *core.BlockChain) *Miner {
	miner := &Miner{
		eth:          eth,
		mux:          mux,
		engine:       engine,
	...
	miner.consensus, _ = recfg.NewConsensus(config.OnetTomlDir, config.OnetGroupPublicKeyDir, txblockchain)
	miner.Register(NewCpuAgent(eth.BlockChain(), engine, miner.consensus)) //leader will call this
