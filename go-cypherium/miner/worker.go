...
import (
	...
	"github.com/cypherium/CypherTestNet/go-cypherium/consensus/misc"
...
type ReCommitNewWork struct{ Restart bool }
...
type worker struct {
	...
	// update loop
	chainReCommitCh chan *ReCommitNewWork
	mux             *event.TypeMux
...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, eth Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		...
		eth:             eth,
		mux:             mux,
		chainReCommitCh: make(chan *ReCommitNewWork, 1),
...
func (self *worker) update() {
	...
	for {
		...
		select {
		case ev := <-self.chainReCommitCh:
			if ev.Restart == true && atomic.LoadInt32(&self.mining) == 1 {
				self.commitNewWork()
			}
...
func (self *worker) commitNewWork() {
	...
	*/
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		if work.tcount != 0 {
			log.Info("Commit new consensus work", "number", work.Block.Number(), "txs", work.tcount, "elapsed", common.PrettyDuration(time.Since(tstart)))
		}
