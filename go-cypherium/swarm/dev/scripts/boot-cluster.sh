...
#
# The cluster contains a bootnode, a cypher node and multiple swarm nodes, with
...
BOOTNODE_URL="cnode://${BOOTNODE_PUBKEY}@${BOOTNODE_IP}:${BOOTNODE_PORT}"

# static cypher configuration
...
# start_bootnode starts a bootnode which is used to bootstrap the cypher and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a cypher node with --datadir pointing at <base-dir>/cypher
# and a single, unlocked account with password "cypher"
start_geth_node() {
  local dir="${base_dir}/cypher"
  mkdir -p "${dir}"

  local password="cypher"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating cypher account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get cypher account address"
  fi
  ...
  start_node "cypher" "${GETH_IP}" "$(which cypher)" ${args[@]}
}
...
create_account() {
  ...
  cypher --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
