...
// viewChange message
// @Public : kyber.Point who currently send viewchange message
// @SeqNo  : sequence number which auto increases one per a consensus round
// @Verifydata : sign with public key and the SeqNo
type ViewChange struct {
	Public     kyber.Point
	SeqNo      uint64
	Verifydata []byte

	//TODO: will we need include current committee??

	//PrimaryID onet.TreeNodeID
	//currently who send viewchange
	//SenderID onet.TreeNodeID
}

// viewchangeChan is the type of the channel used to catch the viewchange messages
type viewchangeChan struct {
	*onet.TreeNode
	ViewChange
}
...
var consensus1 Consensus
var consensus2 Consensus
