...
# migration.
CYPHER_PATH="/e/golang/src/github.com/cypherium_private/go-cypherium/cmd/cypher"
...
runLocal(){
	...
	for n in $( seq $NBR ); do
		...
		if [ -f $co/public.toml ]; then
			...
			if grep 'Public =' $co/public.toml|grep -q =\"; then
				...
				$CYPHER_PATH/debug convert64 < $co/public.toml.bak > $co/public.toml
			fi
		fi

		if [ ! -d $co ]; then
			echo -e "localhost:$((7000 + 2 * $n))\nConode_$n\n$co" | $CYPHER_PATH/debug setup
		fi
        rm -rf $co/debug
        $CYPHER_PATH/debug --datadir $co init $GEN/genesis.json
	...
	for n in $( seq $NBR ); do
		co=co$n
		$CYPHER_PATH/debug --onetdebug $DEBUG --onetdir $co/private.toml --publickeydir public.toml --datadir $co --networkid 123666 --port $((5000 + 2 * $n)) --rpcport $((8000 + 2 * $n)) --verbosity 4 --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi eth,web3,personal --nodiscover 2>$co/output.log &
	done
