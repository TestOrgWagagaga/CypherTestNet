...
const sendTx = async (web3, from, to, amount, n) => {
...
const sendTxns2 = async (web3, from, to, amount, start, number) => {
    const nonceArray = fillRange(start, start + number - 1)

    const results = await Promise.all(nonceArray.map( (nonce) => {
        return sendTx(web3, from, to, amount, nonce).catch(x=> {
            console.log(`rejected nonce: ${nonce}`)
        })
    }))
...
const sendTxns3 =  (web3, from, to, amount, start, number) => {
    const nonceArray = fillRange(start, start + number - 1)
...
const showAccountAndBalance = async (web3) => {
    let accounts = web3.cph.accounts

    let balance = accounts.map((account) => {
            return {
                account,
                balance: web3.fromWei(web3.cph.getBalance(account).toNumber(), 'ether')
            }
        }
    )
...
const testInfuraRopsten = async () => {
    ...
    if(!web3.isConnected()){
        throw new Error('unable to connect to ethereum node at ' + infuraNode)
    }

    const latest = await web3.eth.blockNumber
...
const testTransaction = async () => {
    ...
    if(!web3.isConnected()){
        throw new Error('unable to connect to ethereum node at ' + node)
    }

    const nonce = await web3.cph.getTransactionCount(from)
    console.log(`nonce start from ${nonce}`)

    await web3.personal.unlockAccount(from, "1", 10000)

    const numberOfTxs = 100
    for( startNonce = nonce; startNonce < nonce + 1000 - 1; startNonce++){

        await sendTxns(web3, from, to, 0.01, startNonce, numberOfTxs)
...
const listAccount = async () => {
    ...
    await showAccountAndBalance(web3)
}
...
const testBatch = async () => {
    ...
    await web3.cph.batchTransaction(tx, numberToSend)
...
const testAutoTransaction = async () => {
    ...
    await web3.personal.unlockAccount(from, "1", 0)     // until cypher exits

    const result = await web3.cph.autoTransaction(run, from, to)
    console.log(result)
}
